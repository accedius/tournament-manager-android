...
public class MatchManager extends TManager<Match> implements IMatchManager {
    ...
    public List<Match> getByTournamentId(long tournamentId) {
        try {
            List<Match> matches = SquashManagerFactory.getInstance().getDaoFactory().getMyDao(Match.class).queryForEq(DBConstants.cTOURNAMENT_ID, tournamentId);
            for (Match match : matches) {
                IParticipantManager participantManager = SquashManagerFactory.getInstance().getEntityManager(Participant.class);
    ...
    public void generateRound(long tournamentId) {
        ITournamentManager tournamentManager = SquashManagerFactory.getInstance().getEntityManager(Tournament.class);
        ITeamManager teamManager = SquashManagerFactory.getInstance().getEntityManager(Team.class);
        Tournament tournament = tournamentManager.getById(tournamentId);
        Competition competition = SquashManagerFactory.getInstance().getEntityManager(Competition.class).getById(tournament.getCompetitionId());
        ...
        if (CompetitionTypes.individuals().equals(type)) {
            List<Player> players = tournamentManager.getTournamentPlayers(tournamentId);
    ...
    public void resetMatch(long matchId) {
        ...
        try {
            ...
            for (Participant participant : participants) {
                DeleteBuilder<ParticipantStat, Long> participantStatBuilder = SquashManagerFactory.getInstance().getDaoFactory().getMyDao(ParticipantStat.class).deleteBuilder();
                participantStatBuilder.where().eq(DBConstants.cPARTICIPANT_ID, participant.getId());
                participantStatBuilder.delete();

                DeleteBuilder<PlayerStat, Long> playerStatBuilder = SquashManagerFactory.getInstance().getDaoFactory().getMyDao(PlayerStat.class).deleteBuilder();
    ...
    public void insert(Match match) {
        ...
        Tournament tournament = SquashManagerFactory.getInstance().getEntityManager(Tournament.class).getById(match.getTournamentId());
        Competition competition = SquashManagerFactory.getInstance().getEntityManager(Competition.class).getById(tournament.getCompetitionId());
        ...
        for (Participant participant : match.getParticipants())
            SquashManagerFactory.getInstance().getEntityManager(Participant.class).insert(participant);

        Map<Long, Team> teamMap = new HashMap<>();
        ITeamManager teamManager = SquashManagerFactory.getInstance().getEntityManager(Team.class);
        List<Team> teams = teamManager.getByTournamentId(match.getTournamentId());
        for (Team team : teams) {
        ...
        for (PlayerStat playerStat : playerStats)
            SquashManagerFactory.getInstance().getEntityManager(PlayerStat.class).insert(playerStat);
    }
    ...
    public boolean delete(long id) {
        ...
        try {
            for (Participant participant : participants) {
                ...
                DeleteBuilder<PlayerStat, Long> statBuilder = SquashManagerFactory.getInstance().getDaoFactory().getMyDao(PlayerStat.class).deleteBuilder();
                statBuilder.where().eq(DBConstants.cPARTICIPANT_ID, participant.getId());
                statBuilder.delete();
            }
            DeleteBuilder<Participant, Long> participantBuilder = SquashManagerFactory.getInstance().getDaoFactory().getMyDao(Participant.class).deleteBuilder();
    ...
    public void updateMatch(long matchId, ArrayList<SetRowItem> sets) {
        ...
        IParticipantStatManager participantStatManager = SquashManagerFactory.getInstance().getEntityManager(ParticipantStat.class);
        participantStatManager.deleteByMatchId(matchId);
        ...
        for (ParticipantStat stat : stats)
            participantStatManager.insert(stat);
    }
    ...
    private String getParticipantName(Match match, Participant participant) {
        ...
        if (CompetitionTypes.teams().equals(competition.getType())) {
            Team team = SquashManagerFactory.getInstance().getEntityManager(Team.class).getById(participant.getParticipantId());
            return team.getName();
        } else {
            Player player = SquashManagerFactory.getInstance().getEntityManager(Player.class).getById(participant.getParticipantId());
    ...
    private void loadMatchScore(Match match) {
        ...
        IParticipantStatManager participantStatManager = SquashManagerFactory.getInstance().getEntityManager(ParticipantStat.class);
        List<ParticipantStat> homeStats = participantStatManager.getByParticipantId(home.getId());
        List<ParticipantStat> awayStats = participantStatManager.getByParticipantId(away.getId());
