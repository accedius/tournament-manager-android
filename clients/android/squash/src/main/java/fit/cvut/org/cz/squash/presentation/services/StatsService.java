    ...
    protected void doWork(Intent intent) {
        ...
        switch (action){
            case ACTION_GET_STATS_BY_COMPETITION:{
                ...
                List<SAggregatedStats> stats = ((IStatisticManager)ManagerFactory.getInstance(this).getEntityManager(SAggregatedStats.class))
                        .getByCompetitionId(intent.getLongExtra(ExtraConstants.EXTRA_ID, -1));
                result.putExtra(ExtraConstants.EXTRA_STATS, new ArrayList<>(stats));
            ...
            case ACTION_GET_STATS_BY_TOURNAMENT:{
                ...
                ArrayList<SAggregatedStats> stats = new ArrayList<>(((IStatisticManager)ManagerFactory.getInstance(this).getEntityManager(SAggregatedStats.class))
                        .getByTournamentId(intent.getLongExtra(ExtraConstants.EXTRA_ID, -1)));
                result.putExtra(ExtraConstants.EXTRA_STATS, stats);
                LocalBroadcastManager.getInstance(this).sendBroadcast(result);
                break;
            }
            case ACTION_GET_STANDINGS:{
                ...
                Intent result = new Intent(action);
                ArrayList<StandingItem> standingItems = new ArrayList<>(((IStatisticManager)ManagerFactory.getInstance(this).getEntityManager(SAggregatedStats.class))
                        .getStandingsByTournamentId(intent.getLongExtra(ExtraConstants.EXTRA_ID, -1)));
                result.putExtra(ExtraConstants.EXTRA_STATS, standingItems);
                LocalBroadcastManager.getInstance(this).sendBroadcast(result);
                break;
            }
        }
